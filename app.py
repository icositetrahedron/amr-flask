...
def get_user(id):
    with open("users.pickle","rb") as f:
        return dill.load(f)[str(id)]

def set_sentences(sentences):
    users = dill.load(open("sentences.pickle","rb"))
...
def get_current_sentence():
    return get_sentences()[current_user.current_sentence_id-1]
...
def index():
    if request.method == 'POST':
        if "set_sentence" in request.form:
            ...
            current_user.set_current_sentence_id(request.form["sentence_id"])
...
def parse_command(raw_command):
    ...
    sentences[current_user.current_sentence_id-1].parse_command(raw_command)
...
def set_verb_sense(sense):
    sense = int(sense)
    sentences = get_sentences()
    sentence = sentences[current_user.current_sentence_id-1]
...
def login():
    ...
    if request.method == 'POST':
        user_id = db.get_db().execute(
            'SELECT id FROM users WHERE username = ?', (request.form['username'],)
        ).fetchone()
        if user_id is None:
